...
**Architecture:** A v3 visual kind is declared in six places and consumed in four. Declared: the plan schema (`dsl/teaching_plan.py`), the frozen program schema (`dsl/scene_program.py`), the resolver's expression-evaluation branch, the measurement factory registry (`v3/visual_registry.py`), the compiler's part/expression maps, and `v3/visual_registry.py`'s `_SIZE_DRIVING_FIELDS` -- the fields `_require_renderable_extent` names in a too-large-to-render failure, which a `unit_tape` bug fix (its `value`/`per_unit`/unit-string fields were missing) revealed was not on this list even though the other five sites already accounted for the kind. Consumed: the beat expander (which program-visual class and strategy staging), the static quality gate, the renderer, and the render probe. Every gate runs before a draft is persisted, so a kind that compiles but cannot render surfaces as `needs_manual_authoring`, never as a broken draft.
