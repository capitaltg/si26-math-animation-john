...
_DRAFT_SYSTEM_PROMPT = (
    ...
    "resolves to the value only at conclude. Emit exactly one conclude beat, "
    "and it must be the last beat. Name the unit of the result in "
    "answer_unit (\"meters\"; empty if unitless). Never author a label or callout "
    "standing in for the answer, and never put \"?\" in a label or callout text. "
    "Simple collections reveal together. Perimeter explanations use "
    "boundary_trace on a rectangle_measurement alone; the traced boundary IS "
    "the derivation, so do not add a second structural supporting_visual "
    "(number_line, bar, grid, partition, object_set, unit_tape, "
    "coordinate_plane, or data_display) beside it -- label companions for a "
    "formula are fine. "
    "coordinate_plane also hosts a polygon and pivot for the `rotation` "
    "strategy -- pick this archetype for 'rotate about a point N times' "
    "questions. For rotation, expose only the iteration count as a param "
    "(one integer field grounded in the excerpt's stated count of turns); "
    "the polygon vertices, pivot, plane extents, and rotation angle must "
    "be literal nodes in the teaching_plan_document, not fields, since "
    "the excerpt does not state their numeric values and inventing param "
    "values that are not grounded in the source rejects the draft. "
    "Rotation beats target the coordinate_plane by visual_ref alone (no "
    "`part`, no `index`): the polygon and pivot are not indexed semantic "
    "parts of the plane, and `point` is empty when no points are declared, "
    "so a target that names a part on a rotation plane fails the compiler. "
    "The rotated image itself IS the answer; rotation plans declare no "
    "supporting_visuals, every beat targets only the primary coordinate_plane, "
    "and there is no separate computed-total or supporting-label beat. Keep "
    "the plan to about three beats -- one to reveal the starting position, "
    "the single focus-or-derive beat the compiler stages the rotation on, "
    "and a conclude beat that lands on the final image. "
    "Median ordered values use item-specific targets. "
    ...
    "is a scale rather than a part count, or on a unit_tape when the lesson "
